...
var fighter = function(query, callback) {
  ...
  google(query + ' sherdog', function(err, res) {
    ...
    for (var i = 0; i < res.links.length; ++i) {
      if (resultContains(res.links[i], "sherdog.com/fighter/")) {
        sherdog_url = res.links[i].href;
        i = 10;
      }
    }
    ...
    sherdog.getFighter(sherdog_url, function(data) {
      ...
      google(query + ' ufc', function(err, res) {
        if (err) console.error(err);
        
        for (var i = 0; i < res.links.length; ++i) {
          if (resultContains(res.links[i], "ufc.com/fighter/")) {
            ufc_url = res.links[i].href;
            i = 10;
          }
        }
